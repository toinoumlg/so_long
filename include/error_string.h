/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   error_string.h                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: amalangu <amalangu@student.42lyon.fr>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/07 22:58:01 by amalangu          #+#    #+#             */
/*   Updated: 2025/07/08 15:18:48 by amalangu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef ERROR_STRING_H
# define ERROR_STRING_H

# define FILE_DOESNT_EXIST "file doesn't exist."
# define FILE_EMPTY "file is empty."
# define WRONG_BORDERS "border don't contain only walls."
# define INVALID_EXTENSION "invalid file extension, should end with .ber."
# define MAP_NOT_RECTANGULAR "map isn't rectangular."
# define ALLOC_ERROR "allocation error."
# define MULTIPLE_EXITS "map have multiples exits."
# define MULTIPLE_PLAYERS "map have multiples players."
# define WRONG_CHARACTER "a character isn't either: [0, 1, 2, P, E, C]."
# define NO_PATH_AVAILABLE "no available path found"

#endif